...
@media (max-width: 600px) {
  ...
  .topbar-nav .nav-btn { padding: 7px 12px; font-size: 10px; }
    ...
    .deck-tabs { display:flex; gap:3px; padding:10px 14px; background:var(--bg-panel); border-bottom:1px solid var(--border); }
    .deck-tab  { padding:6px 16px; background:var(--bg-card); border:1px solid var(--border); border-radius:3px; font-family:'Oxanium',monospace; font-size:11px; font-weight:700; color:var(--text-dim); cursor:pointer; transition:all 0.15s; }
    .deck-tab.active { background:var(--red); color:var(--white); border-color:var(--red); }
    .deck-tab:hover:not(.active) { border-color:var(--red); color:var(--white); }

    .deck-builder-layout { display:grid; grid-template-columns:1fr 260px; gap:14px; padding:14px; }

    .deck-slots-panel { background:var(--bg-panel); border:1px solid var(--border); border-radius:4px; overflow:hidden; }
    .deck-slots-header { background:var(--red-dark); padding:10px 14px; display:flex; align-items:center; justify-content:space-between; }
    .deck-slots-title { font-family:'Oxanium',monospace; font-size:13px; font-weight:700; color:var(--white); }
    .deck-name-form { display:flex; gap:8px; align-items:center; }
    .deck-name-input { background:rgba(0,0,0,0.3); border:1px solid rgba(255,255,255,0.15); border-radius:3px; color:var(--white); font-family:'Oxanium',monospace; font-size:14px; font-weight:700; padding:5px 10px; outline:none; }

    .card-slots-grid { display:grid; grid-template-columns:repeat(5,1fr); gap:8px; padding:14px; }
    .card-slot { aspect-ratio:2/3; border-radius:6px; border:1px dashed rgba(255,255,255,0.1); display:flex; flex-direction:column; align-items:center; justify-content:center; font-size:11px; color:rgba(255,255,255,0.15); cursor:pointer; transition:all 0.2s; position:relative; min-height:66px; background:rgba(0,0,0,0.15); }
    .card-slot.filled { border-style:solid; border-color:var(--border); }
    .card-slot.filled:hover { border-color:rgba(204,26,26,0.5); }
    .card-slot:not(.filled):hover { background:rgba(255,255,255,0.03); border-color:rgba(255,255,255,0.2); }
    .slot-remove { position:absolute; top:3px; right:3px; width:14px; height:14px; border-radius:50%; background:rgba(204,26,26,0.8); color:#fff; font-size:8px; display:none; align-items:center; justify-content:center; cursor:pointer; font-weight:800; border:none; }
    .card-slot.filled:hover .slot-remove { display:flex; }
    .slot-emoji { font-size:20px; line-height:1; }
    .slot-name  { font-size:7px; font-weight:700; color:var(--text-dim); margin-top:3px; text-align:center; padding:0 2px; line-height:1.2; }

    .card-picker { background:var(--bg-panel); border:1px solid var(--border); border-radius:4px; overflow:hidden; }
    .picker-header { background:var(--red-dark); padding:8px 12px; font-family:'Oxanium',monospace; font-size:11px; font-weight:700; color:var(--white); }
    .picker-list   { max-height:420px; overflow-y:auto; }
    .picker-item   { display:flex; align-items:center; gap:10px; padding:8px 12px; border-bottom:1px solid var(--border); cursor:pointer; transition:background 0.15s; }
    .picker-item:hover { background:rgba(255,255,255,0.04); }
    .picker-emoji  { font-size:18px; flex-shrink:0; }
    .picker-info   { flex:1; min-width:0; }
    .picker-name   { font-size:11px; font-weight:800; color:var(--white); white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
    .picker-type   { font-size:9px; color:var(--text-dim); }
    .picker-add    { font-size:14px; color:var(--teal); font-weight:800; flex-shrink:0; }

    .deck-progress-bar { height:4px; background:var(--border); margin:0; }
    .deck-progress-fill { height:4px; background:linear-gradient(90deg,var(--red),var(--red-light)); transition:width 0.4s; }
  </style>
</head>
